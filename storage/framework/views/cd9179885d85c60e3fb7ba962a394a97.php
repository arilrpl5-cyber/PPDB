...
<div class="mb-8">
    <div class="flex items-center space-x-4 mb-4">
        <div>
            <h1 class="text-3xl font-extrabold text-[#1E3A8A] tracking-tight">Portal Penerimaan Siswa Baru</h1>
...
<div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-6">
                <div class="flex flex-col items-center justify-center mb-6">
                    ...
                    <div class="text-xl font-bold text-gray-800 text-center"><?php echo e($user->name); ?></div>
                    <div class="text-sm font-medium text-gray-500 text-center"><?php echo e($user->email); ?></div>
                </div>

                <div class="space-y-4 pt-4 border-t border-gray-100">
                    <div class="flex justify-between items-center py-2">
                        <span class="text-sm font-bold text-gray-500 w-1/3">No. Test</span>
                        <span class="text-sm font-black text-[#0F172A] flex-1 text-right bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100"><?php echo e($profile->test_number ?? 'Belum Digenerate'); ?></span>
                    </div>
                    <div class="flex justify-between items-center py-2">
                        <span class="text-sm font-bold text-gray-500 w-1/3">No. Telp</span>
                        <span class="text-sm font-bold text-[#0F172A] flex-1 text-right"><?php echo e($profile->phone ?? '-'); ?></span>
                    </div>
                    <?php if($studentId): ?>
                    <div class="flex justify-between items-center py-2">
    ...
    <div class="md:col-span-7 space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden relative group">
            <div class="absolute inset-0 bg-gradient-to-r <?php echo e($profile->test_status === 'SELESAI' ? 'from-emerald-500/5 to-transparent' : 'from-amber-500/5 to-transparent'); ?> opacity-50"></div>
            
            <div class="p-6 border-b border-gray-100 relative z-10 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <div class="p-2 <?php echo e($profile->test_status === 'SELESAI' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'); ?> rounded-lg">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
                    </div>
                    <h3 class="text-xl font-extrabold text-[#0F172A]">Status Seleksi Masuk</h3>
                </div>
                
                <?php if($profile->test_status === 'SELESAI'): ?>
            ...
            <div class="p-6 relative z-10">
                <?php if($profile->test_status !== 'SELESAI'): ?>
                    ...
                    <form action="<?php echo e(route('student.exam.show')); ?>" method="GET" class="w-full">
                ...
                <?php else: ?>
                    ...
                    <?php if(($result->is_passed ?? 'PENDING') === 'PENDING'): ?>
                        <div class="text-center py-6">
                            ...
                            <p class="text-sm text-gray-500 font-medium">Kami telah menerima lembar jawaban Anda dengan perolehan skor awal <span class="font-extrabold text-[#1E3A8A]"><?php echo e($result->total_score ?? 0); ?></span>. Silakan tunggu review final kelulusan dari Panitia PPDB.</p>
                        </div>
                    <
                    <?php elseif($result->is_passed === 'LULUS'): ?>
                        <div class="text-center py-4">
                            ...
                            <p class="text-sm text-gray-600 font-medium mb-4">Selamat! Anda dinyatakan lolos seleksi dengan skor total <span class="font-extrabold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded"><?php echo e($result->total_score); ?></span>. Silakan lanjut ke proses Daftar Ulang di bawah.</p>
                            <?php if($result->admin_notes): ?>
                            <div class="bg-gray-50 rounded-lg p-3 text-xs text-left italic font-medium text-gray-500 border border-gray-200">
                                " <?php echo e($result->admin_notes); ?> " - Admin PPDB
                            </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <?php else: ?>
                        <div class="text-center py-4">
                            ...
                            <p class="text-sm text-gray-600 font-medium mb-4">Mohon maaf, berdasarkan evaluasi dengan skor <span class="font-extrabold text-red-700 bg-red-50 px-2 py-0.5 rounded"><?php echo e($result->total_score ?? 0); ?></span>, Anda dinyatakan tidak lulus seleksi kali ini.</p>
                            <?php if($result->admin_notes): ?>
                            <div class="bg-gray-50 rounded-lg p-3 text-xs text-left italic font-medium text-gray-500 border border-gray-200">
                                " <?php echo e($result->admin_notes); ?> " - Admin PPDB
                            </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gray-50 flex items-center space-x-3">
                ...
                <h3 class="text-xl font-extrabold text-[#0F172A]">Daftar Ulang Peserta Didik Baru</h3>
            </div>
            
            <div class="p-6">
                <?php if(!$reRegistration): ?>
                    <form action="<?php echo e(route('student.re-register')); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                        <?php echo csrf_field(); ?>
                ...
                <?php else: ?>
                    <div class="relative overflow-hidden bg-white border <?php echo e($reRegistration->status === 'DISETUJUI' ? 'border-emerald-200 shadow-emerald-100/50' : ($reRegistration->status === 'DITOLAK' ? 'border-red-200 shadow-red-100/50' : 'border-blue-200 shadow-blue-100/50')); ?> shadow-lg rounded-2xl p-6">
                        <div class="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 rounded-full opacity-10 <?php echo e($reRegistration->status === 'DISETUJUI' ? 'bg-emerald-500' : ($reRegistration->status === 'DITOLAK' ? 'bg-red-500' : 'bg-blue-500')); ?>"></div>
                        
                        <div class="flex items-center mb-4">
                            <?php if($reRegistration->status === 'DISETUJUI'): ?>
                                <div class="p-2 bg-emerald-100 text-emerald-600 rounded-lg mr-4">
                                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                </div>
                            <?php elseif($reRegistration->status === 'DITOLAK'): ?>
                        ...
                        <?php if($reRegistration->status === 'DISETUJUI'): ?>
                            <p class="text-sm text-gray-600 font-medium leading-relaxed">Selamat! Berkas dan registrasi ulang Anda telah diverifikasi oleh Panitia PPDB. Anda telah resmi menjadi Peserta Didik Baru. <strong class="text-[#1E3A8A]">Admin akan meng-generate dan menginformasikan NIM Anda segera melalui portal ini.</strong> Pastikan untuk selalu memeriksa bagian "Profil Pendaftar" di sebelah kiri untuk melihat update NIM Anda.</p>
                        <?php elseif($reRegistration->status === 'DITOLAK'): ?>
                            <p class="text-sm text-gray-600 font-medium leading-relaxed">Mohon maaf, berkas daftar ulang Anda ditolak oleh panitia pendaftaran. Silahkan hubungi panitia PPDB melalui kontak layanan kampus untuk informasi lebih lanjut mengenai masalah ini.</p>
                        <?php else: ?>
                            <p class="text-sm text-gray-600 font-medium leading-relaxed">Pemberkasan Anda berhasil diunggah dan sedang dalam antrean pengecekan panitia PPDB. Harap untuk mengecek portal secara berkala guna melihat perubahan status registrasi ulang maupun penerbitan **NIS** Anda.</p>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
...
<?php echo $__env->make('layouts.student', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\RPL-04\Downloads\PMB-Laravel-10-main\PMB-Laravel-10-main\resources\views\student\dashboard.blade.php ENDPATH**/ ?>