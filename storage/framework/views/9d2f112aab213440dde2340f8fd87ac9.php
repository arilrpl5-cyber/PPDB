...
<body class="bg-[#F8FAFC] font-sans text-gray-900 select-none overflow-x-hidden" oncontextmenu="return false;">
    <script>
        ...
        window.EXAM_TOKEN = "<?php echo e($session->exam_token); ?>";
    </script>
    ...
    <div class="fixed top-0 w-full bg-white shadow-md shadow-blue-900/5 py-4 px-6 z-50 flex flex-col md:flex-row justify-between items-center border-t-4 border-[#2563EB]">
        <div class="flex items-center space-x-3 mb-2 md:mb-0">
            ...
            <div>
                <div class="font-black text-lg text-[#0F172A] leading-tight">PPDB UJIAN ONLINE</div>
        ...
        <div class="flex items-center space-x-6">
            ...
            <form action="<?php echo e(route('student.exam.finish')); ?>" method="POST" id="finishForm">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="exam_token" value="<?php echo e($session->exam_token); ?>">
                ...
                <div class="bg-white shadow-xl shadow-blue-900/5 rounded-2xl border border-gray-100 sticky top-32 overflow-hidden">
                    ...
                    <div class="p-4">
                        <div class="grid grid-cols-5 gap-2">
                            <?php $__currentLoopData = $questionsData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                                $isAnswered = isset($existingAnswers[$q['id']]);
                            ?>
                            <button @click="currentQuestion = <?php echo e($index); ?>" 
                                class="w-full aspect-square rounded-xl flex items-center justify-center font-bold text-sm transition-all shadow-sm
                                <?php echo e($isAnswered ? 'bg-gradient-to-br from-indigo-500 to-[#1E3A8A] text-white border-transparent' : 'bg-white text-gray-600 border border-gray-200 hover:border-indigo-400 hover:bg-indigo-50'); ?>" 
                                :class="{'ring-4 ring-indigo-200 ring-offset-1 scale-105 transform': currentQuestion === <?php echo e($index); ?>}">
                                <?php echo e($index + 1); ?>
            ...
            <div class="lg:col-span-8 order-1 lg:order-2">
                <div class="bg-white shadow-2xl shadow-blue-900/5 rounded-3xl border border-gray-100 overflow-hidden relative">
                    ...
                    <div class="h-1.5 w-full bg-gray-100 relative">
                        <div class="absolute top-0 left-0 h-full bg-[#2563EB] transition-all duration-300" :style="`width: ${((currentQuestion + 1) / <?php echo e(count($questionsData)); ?>) * 100}%`"></div>
                    </div>

                    <div class="p-5 md:p-8">
                        <?php $__currentLoopData = $questionsData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div x-show="currentQuestion === <?php echo e($index); ?>" class="question-block" 
                                ...
                                <div class="mb-6 flex justify-between items-center bg-blue-50/50 p-3 rounded-xl border border-blue-50">
                                    <span class="text-[#1E3A8A] font-black tracking-wide">PERTANYAAN NO. <?php echo e($index + 1); ?></span>
                                    <span class="text-xs font-bold text-gray-400">Total <?php echo e(count($questionsData)); ?> Soal</span>
                                </div>
                                ...
                                <div class="space-y-4">
                                    <?php $__currentLoopData = $q['options']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php
                                    ...
                                    <label class="group flex items-start p-4 md:p-5 border-2 rounded-2xl cursor-pointer transition-all duration-200 
                                            <?php echo e($isChecked ? 'bg-indigo-50/80 border-[#2563EB]' : 'bg-white border-gray-100 hover:border-indigo-300 hover:bg-gray-50'); ?>">
                                        
                                        <div class="relative flex items-center justify-center mt-0.5">
                                            <input type="radio" name="answer_<?php echo e($q['id']); ?>" value="<?php echo e($key); ?>" 
                                                class="peer sr-only answer-radio" data-qid="<?php echo e($q['id']); ?>" <?php echo e($isChecked ? 'checked' : ''); ?> 
                                                onclick="saveAnswer(<?php echo e($q['id']); ?>, '<?php echo e($key); ?>')">
                                        ...
                                        <span class="ml-4 text-[15px] font-medium text-gray-700 group-hover:text-gray-900 <?php echo e($isChecked ? 'text-[#0F172A] font-bold' : ''); ?> flex-1">
                                            <span class="font-extrabold mr-1.5 text-gray-400 group-hover:text-indigo-400 <?php echo e($isChecked ? 'text-[#2563EB]' : ''); ?>"><?php echo e($key); ?>.</span> 
                                            <?php echo e($option); ?>
                                ...
                                <div class="flex justify-between items-center mt-10 pt-6 border-t border-gray-100">
                                    ...
                                    <button type="button" @click="if(currentQuestion < <?php echo e(count($questionsData) - 1); ?>) currentQuestion++" 
                                        ...
                                        :disabled="currentQuestion === <?php echo e(count($questionsData) - 1); ?>">
                                        Selanjutnya
    ...
    <script>
        ...
        // Timer
        const endTime = new Date("<?php echo e($session->end_time->toIso8601String()); ?>").getTime();
        ...
        function saveAnswer(qId, answer) {
            $.post("<?php echo e(route('student.exam.save')); ?>", { question_id: qId, answer: answer })
             .fail(function(xhr) {
        ...
        function triggerCheatLog(type) {
            ...
            $.post("<?php echo e(route('student.exam.log')); ?>", { log_type: type })
             .done(function(data) {
                 if(data.locked) {
