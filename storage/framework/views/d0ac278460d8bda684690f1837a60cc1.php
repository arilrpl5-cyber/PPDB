...
<div class="mb-8">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 font-medium">Kelola data calon siswa baru, nilai ujian, kelulusan, dan status daftar ulang.</p>
        </div>
    </div>
</div>
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-blue-50/50 transition-colors group">
                    ...
                    <!-- Status Ujian -->
                    <td class="px-6 py-5 whitespace-nowrap">
                        <?php if($user->profile?->test_status === 'SELESAI'): ?>
                            <div class="flex items-center space-x-2">
                                <span class="bg-blue-100 text-blue-800 text-xs font-bold px-2.5 py-1 rounded-full border border-blue-200">
                                    Selesai
                                </span>
                                <span class="text-sm font-extrabold text-[#0F172A]">Nilai: <?php echo e($user->result->total_score ?? 0); ?></span>
                            </div>
                        <?php else: ?>
                    ...
                    <!-- Kelulusan -->
                    <td class="px-6 py-5 whitespace-nowrap">
                        <form action="<?php echo e(route('admin.users.evaluate', $user->id)); ?>" method="POST" class="flex items-center space-x-2">
                            <?php echo csrf_field(); ?>
                            <select name="status" class="bg-gray-50 border border-gray-200 text-gray-700 text-xs rounded-lg focus:ring-[#2563EB] focus:border-[#2563EB] block w-full p-2" <?php echo e($user->profile?->test_status !== 'SELESAI' ? 'disabled' : ''); ?>>
                                <option value="PENDING" <?php echo e(($user->result->is_passed ?? '') === 'PENDING' ? 'selected' : ''); ?>>Menunggu Review</option>
                                <option value="LULUS" <?php echo e(($user->result->is_passed ?? '') === 'LULUS' ? 'selected' : ''); ?>>✅ Lulus</option>
                                <option value="TIDAK LULUS" <?php echo e(($user->result->is_passed ?? '') === 'TIDAK LULUS' ? 'selected' : ''); ?>>❌ Tidak Lulus</option>
                            </select>
                            <button type="submit" class="text-white bg-[#1E3A8A] hover:bg-[#172e6b] focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-xs px-3 py-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors" <?php echo e($user->profile?->test_status !== 'SELESAI' ? 'disabled' : ''); ?>>
                                Set
                            </button>
                        </form>
                    </td>
                        ...
                        <?php if($user->reRegistration): ?>
                            <div class="flex flex-col space-y-2">
                                <div class="flex items-center justify-between space-x-2">
                                    ...
                                    ?>
                                    <span class="text-[10px] font-bold px-2 py-0.5 rounded border <?php echo e($statusClass); ?>">
                                        <?php echo e($user->reRegistration->status); ?>

                                    </span>
                                    
                                    <?php if($user->reRegistration->payment_proof): ?>
                                        <a href="<?php echo e(Storage::url($user->reRegistration->payment_proof)); ?>" target="_blank" class="text-xs font-medium text-[#2563EB] hover:underline flex items-center">
                                ...
                                <form action="<?php echo e(route('admin.users.approve_re_registration', $user->id)); ?>" method="POST" class="flex items-center space-x-2">
                                    <?php echo csrf_field(); ?>
                                    <select name="status" class="bg-gray-50 border border-gray-200 text-gray-700 text-xs rounded-lg focus:ring-[#2563EB] focus:border-[#2563EB] block w-full p-2">
                                        <option value="MENUNGGU" <?php echo e($user->reRegistration->status === 'MENUNGGU' ? 'selected' : ''); ?>>🔍 Evaluasi</option>
                                        <option value="DISETUJUI" <?php echo e($user->reRegistration->status === 'DISETUJUI' ? 'selected' : ''); ?>>✅ Sahkan</option>
                                        <option value="DITOLAK" <?php echo e($user->reRegistration->status === 'DITOLAK' ? 'selected' : ''); ?>>❌ Tolak</option>
                                    </select>
                        ...
                        <div class="flex flex-col space-y-2">
                            <?php if($user->studentId): ?>
                                <div class="bg-indigo-50 border border-indigo-100 p-2 rounded-lg text-center">
                                    ...
                                    <span class="font-mono font-black text-[#1E3A8A] text-sm"><?php echo e($user->studentId->nim); ?></span>
                                </div>
                            <?php elseif($user->reRegistration && $user->reRegistration->status === 'DISETUJUI'): ?>
                                <form action="<?php echo e(route('admin.users.generate_nim', $user->id)); ?>" method="POST" class="w-full">
                                    <?php echo csrf_field(); ?>
                            ...
                            <form action="<?php echo e(route('admin.users.destroy', $user->id)); ?>" method="POST" onsubmit="return confirm('PERINGATAN! Anda akan menghapus data pendaftar ini secara permanen. Lanjutkan?');" class="w-full">
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\RPL-04\Downloads\PMB-Laravel-10-main\PMB-Laravel-10-main\resources\views/admin/users/index.blade.php ENDPATH**/ ?>